...
| implementation / verification procedure | `docs/implementation-workflow.md`, `docs/coding-agent-harness.md`, `.agents/skills/execute-task/SKILL.md` | material visual/UI work: `.agents/skills/visual-acceptance/SKILL.md` | task type/riskに応じてprogressive disclosure。reference conformanceはrendered comparison必須 |
| independent acceptance procedure | `.agents/skills/evaluate-change/SKILL.md` + current task contract | visual/UI taskは`.agents/skills/visual-acceptance/SKILL.md`も適用 | full cumulative exact-head candidateをfresh contextで監査。visual PASSをgeneric CIから推測しない |
