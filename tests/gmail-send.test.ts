...
describe('G51 Gmail send boundary', () => {
  ...
  it('serializes a deterministic, threaded text MIME and rejects header injection', () => {
    const built = buildGmailTextMime({sendOperationId: 'operation-1', snapshot: snapshot() as never, originalMessage: gmailMessage('original-provider-1', originalMessageId, {labelIds: ['INBOX']})});
    ...
    expect(raw).toContain('References: <prior@example.com> <original@example.com>');
    expect(raw).toContain('Subject: Re: Request');
