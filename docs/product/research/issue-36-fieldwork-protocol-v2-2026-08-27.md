# Issue #36 Fieldwork Protocol v2 — 2026-08-27
...
- `main`: `d569d8f1a61b45a0d733a53dc2b90ccc37e67bb9`;
- Issue #36 body: current as of `updated_at=2026-08-26T17:19:39Z`;
- Issue #45: complete/closed;
- Product implementation remains unauthorized by this protocol.

Current Product authorities constrain interpretation but are **not participant-facing hypotheses**:
...
3. `docs/product/GOLDEN-SCENARIO-BANK.md` — Product regression bank, subordinate to Responsibility semantics;
...
A positive result requires recent-event evidence that a coherent cohort repeatedly experiences the monitoring burden **after accounting for the tools it actually uses**.

---

# 2. Evidence classes

Use the Issue #36 evidence classes exactly.
...
Every material inference should point back to the relevant cases/evidence classes.

## `UNKNOWN`

Not established.

Do not force an answer where the data do not support one.
...
Anchor the interview in specific participant-selected recent events rather than abstract opinions.
...
The goal is to recover sequence, monitoring behavior, tool use, and consequence—not to map the participant into Lunowa's ontology.

## 3.2 Human moderation for the exploratory discovery wave

The first exploratory wave is **human-moderated**.

Current 2025–2026 evidence supports keeping this boundary:
...
AI must not be treated as an independent validator of:
...
Raw client/email content and raw participant-identifying material must not be sent to an external LLM unless an explicit approved research data-handling decision covers that processor/use.

## 3.3 Gate before any later AI-moderated breadth

Do not silently change interviewer mode and pool results.

If later AI-moderated interviews are considered for breadth after the human guide stabilizes:
...
The bridge is a methodological comparability test, not a competition to prove AI is better/worse.
...
- the envelope is a resourcing reference, not a pass threshold.

Do not mechanically complete 5–6 people in every cohort.

If cohort A rapidly shows a coherent high-information pattern, deepen A rather than spending equal quota on low-value cohorts.

If cohort A is weak, redirect before exhausting a nominal quota.
...
Use to find which workflow characteristics—not title—predict burden.
...
Negative controls test Lunowa's Product jurisdiction and baseline adequacy.

They do **not** count toward saturation of a positive cohort.
...
Do not let a supported cohort rest entirely on one personal network or one panel source.

Cross-source recruitment is a robustness check, not statistical representativeness.

## 5.2 Public recruitment copy

Keep the desired behavior opaque enough to reduce gaming.
...
Current Macromill 2026 operational guidance lists broad Japanese references around:

- 60–90 minute 1:1 depth interview: roughly ¥3,000–¥15,000;
- B2B/expert 60+ minute interview: roughly ¥10,000–¥30,000, with some scarce experts higher.

For this study, a practical **pilot** is:

- standard 45-minute professional interview: ¥8,000–¥10,000;
- scarce/high-opportunity-cost B2B participant: up to roughly ¥12,000–¥15,000 if required by fill-rate evidence.
...
- do not over-emphasize the incentive in public copy;
...
Do not mention Lunowa or the desired monitoring hypothesis.

Before scaled recruitment, run **2–3 target-like pilot comprehension checks** to catch gross ambiguity.

This is **not screener validation or saturation**. Continue revising if real recruitment shows misinterpretation.
...
Open text + categories.

Do not assume feature availability means feature use.
...
Concrete but Product-disconfirming answers are valid.
...
- monitoring is routinely delegated away from the participant;
...
Online incentivized qualitative research has a documented imposter/fraud risk.
...
- probe the concrete event sequence;
- inspect whether tool/workflow details remain internally coherent;
...
Maintain a separate recruitment-integrity log outside Product evidence. Do not place raw identity documents, contact details, IP addresses, or private messages in GitHub.
...
Recording is only used under the pre-fieldwork data-handling decision and explicit consent.

Do not introduce Lunowa solution concepts.
...
Build timeline with neutral probes:
...
For every remembered self-check, ask **why the check occurred**.

If there was no self-check, preserve that as disconfirming evidence.
...
A cautious hypothetical answer does not erase observed recent problem evidence.

A categorical structural reason that safe monitoring could never be delegated may weaken the wedge, but remains self-report until behavior is tested.
...
Artifact use is optional and controlled by the participant.
...
Store only de-identified structured research evidence in the repository.
...
This is critical in 2026 because incumbent email/AI capability changes rapidly.

---

# 12. Analysis process

## 12.1 Within-case first
...
## 12.2 Cohort-specific cross-case analysis
...
## 12.3 Claim-evidence grid

For every material cohort claim maintain:
...
- recruitment repeatedly fails to find the hypothesized behavior.
...
- a coherent workflow-defined segment is becoming clearer.

No universal numeric `N_self_check` or pain threshold is accepted.

---

# 14. Final Issue #36 disposition
...
Hypothetical delegation responses are useful secondary boundary evidence but are not observed adoption.
...
The hypothesized segment/problem does not survive the collected evidence strongly enough to justify continuing the wedge in its current form.

No disposition establishes population prevalence, WTP, retention, PMF, or production reliability.

---

# 15. Data/privacy operation — hard pre-fieldwork gate
...
5. storage location/access controls;
6. participant-code ↔ contact mapping storage, separate from analytical data;
7. raw recording/transcript retention/deletion period;
...
- no names/contact details in GitHub analytical artifacts;
- no credentials/inbox access;
- artifact sharing optional;
- recording off unless explicitly approved and consented;
- if recording is used, delete raw media after structured extraction/verification under the predeclared retention period;
- participant identity mapping remains outside analytical evidence and is deleted when no longer operationally required, subject to actual legal/accounting obligations;
- external LLMs receive only de-identified material unless a separately reviewed processor/data-use decision explicitly permits more;
- participant research data are not repurposed for unrelated marketing/training without a separate appropriate basis/notice.

The Product researcher must check the actual services used against current Japanese Personal Information Protection Commission guidance before collection. This protocol is not legal advice and does not assume a vendor's compliance from marketing claims.

---

# 16. Current external evidence informing the protocol
...
Interpretation: `15–24` can be a useful planning envelope, but support/saturation cannot be manufactured by pooling heterogeneous cohorts.
...
Japan Business Email Association 2026 survey (`n=1,293` work-email users) reports email usage remains near-universal among its respondents, substantial daily checking/time burden, and longer reading/writing time despite fewer received messages.
...
Dillon, Jaffe, Immorlica & Stanton, *Shifting Work Patterns with Generative AI* (66 firms / 7,137 knowledge workers; NBER 2025 revision / AER: Insights forthcoming) reports meaningful email-time savings among users of integrated GenAI.

Interpretation: generic `AI makes email faster` is not a sufficient wedge.

## Current incumbent feature overlap

Current official/product documentation shows substantial overlap at the feature layer:

- Superhuman: auto reminders for unanswered sent mail + auto follow-up drafts;
- Fyxer: awaiting-reply tracking + follow-up drafts after configured wait periods;
...
Fieldwork must record the participant's **actual used baseline**, not assume these features are present, enabled, trusted, or adequate.

## Japan privacy/data handling

Current Personal Information Protection Commission guidance, including 2026 amendments, requires concrete purpose specification/notification, appropriate security management, and actual processor/cloud handling review.

The protocol therefore blocks fieldwork until the actual research data flow is defined.

---

# 17. Pre-fieldwork acceptance checklist

Fieldwork may start only when all required items are true:

- [ ] Current Issue #36 contract rechecked live.
- [ ] Product Content COMPLETE current authorities acknowledged but not exposed as interview hypotheses.
- [ ] Recruitment copy does not reveal desired monitoring behavior.
- [ ] Screener has received 2–3 pilot comprehension checks and obvious ambiguity is corrected.
- [ ] Initial cohort/batch hypothesis is explicit.
- [ ] Negative-control/boundary recruiting remains available.
- [ ] Incentive and eligibility/fraud compensation rules are predeclared operationally.
- [ ] Fraud adjudication flow exists.
- [ ] Human moderator uses neutral recent-event guide for first exploratory wave.
- [ ] Evidence matrix/claim grid is ready before first interview.
- [ ] Participant-facing purpose/privacy/recording notice is ready.
- [ ] Actual data storage/access/retention/deletion policy is filled in.
- [ ] Actual recruiter/video/transcription/cloud/AI processors are reviewed.
- [ ] Raw PII/private email has a storage boundary outside GitHub.
...
# 18. Relationship to downstream work
...
The cheapest next experiment must be selected from the actual Issue #36 evidence rather than predetermined by current Product UI or implementation plans.