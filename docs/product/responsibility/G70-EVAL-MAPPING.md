...
| Draft assistance | `PG-29` plus the independent draft contract tests | editable body only, currentness, no sender/recipient/send fields, manual fallback remains available | G50 draft persistence or G51 provider Send |
...
The executable gate is `pnpm ai:eval:fixtures` (included by `pnpm verify`): it supplies an explicit canonical fixture for every declared case, validates interpretation outputs through schema and runtime derivation, and applies the layer-owned oracle for each case. `tests/ai-runtime.test.ts` independently binds the manifest to the explicit `T0-001`–`T0-044` plus applicable Golden Scenario IDs; runtime provider failure and draft manual fallback remain separate checks for PG-22/PG-29. This remains candidate-layer evidence only and does not claim reducer, scheduler, provider, or Send correctness.
