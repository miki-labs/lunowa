...
/**
 * This is provenance/basis storage only. G70 owns model invocation and the
 * interpretation runtime; inserting this row never invokes a model.
 */
export const aiInterpretationRuns = pgTable(
  'ai_interpretation_runs',
  {
    ...
    providerModelIdentifier: text('provider_model_identifier'),
  ...
  (table) => [
    unique('ai_interpretation_runs_id_user_uq').on(table.id, table.userId),
    foreignKey({
      name: 'ai_interpretation_runs_user_fk',
      columns: [table.userId],
      foreignColumns: [user.id]
    }).onDelete('cascade'),
    foreignKey({
      name: 'ai_interpretation_runs_conversation_fk',
      columns: [table.conversationId],
      foreignColumns: [conversations.id]
    }).onDelete('cascade'),
    foreignKey({
      name: 'ai_interpretation_runs_message_fk',
      columns: [table.messageId],
      foreignColumns: [messages.id]
    }).onDelete('cascade'),
    check('ai_interpretation_runs_schema_version_check', sql`${table.schemaVersion} >= 1`),
    check(
      'ai_interpretation_runs_basis_revision_check',
      sql`${table.basisEvidenceRevision} >= 0`
    ),
    check(
      'ai_interpretation_runs_manifest_object_check',
      sql`jsonb_typeof(${table.contextManifest}) = 'object'`
    ),
    check(
      'ai_interpretation_runs_model_config_version_check',
      sql`char_length(btrim(${table.modelConfigVersion})) BETWEEN 1 AND 128`
    ),
    index('ai_interpretation_runs_user_idx').on(table.userId, table.createdAt)
  ]
);
...
export const responsibilities = pgTable(
  'responsibilities',
  {
    ...
    liveTrackingState: text('live_tracking_state').notNull().default('TRACKING_ACTIVE'),
    attentionMode: text('attention_mode').notNull().default('PRESENT'),
    semanticDetailsVersion: smallint('semantic_details_version').notNull().default(1),
    semanticDetails: jsonb('semantic_details')
      .$type<ResponsibilitySemanticDetailsV1>()
      .notNull()
      .default(jsonObjectDefault),
    acceptedEvidenceRevision: revision('accepted_evidence_revision').notNull(),
  ...
  (table) => [
    ...
    unique('responsibilities_id_account_uq').on(table.id, table.connectedAccountId),
    foreignKey({
    ...
    check('responsibilities_semantic_details_version_check', sql`${table.semanticDetailsVersion} >= 1`),
    check(
    ...
    check('responsibilities_aggregate_version_check', sql`${table.aggregateVersion} >= 1`),
    index('responsibilities_live_open_user_idx')
      .on(table.userId, table.updatedAt.desc(), table.id)
      .where(sql`${table.liveTrackingState} = 'TRACKING_ACTIVE' AND ${table.resolutionStatus} = 'OPEN'`),
    index('responsibilities_live_done_user_idx')
      .on(table.userId, table.resolvedAt.desc(), table.id)
      .where(sql`${table.liveTrackingState} = 'TRACKING_ACTIVE' AND ${table.resolutionStatus} = 'RESOLVED'`),
    index('responsibilities_conversation_idx').on(table.conversationId, table.createdAt, table.id),
...
export const responsibilityExpectedEvents = pgTable(
  ...
  (table) => [
    unique('responsibility_expected_events_id_parent_uq').on(table.id, table.responsibilityId),
    foreignKey({
...
export const responsibilityObligationLegs = pgTable(
  ...
  (table) => [
    unique('responsibility_obligation_legs_id_parent_uq').on(table.id, table.responsibilityId),
    foreignKey({
    ...
    foreignKey({
      ...
      foreignColumns: [responsibilityExpectedEvents.id, responsibilityExpectedEvents.responsibilityId]
    }).onDelete('no action'),
    check(
    ...
    index('responsibility_obligation_legs_open_projection_idx')
      .on(table.responsibilityId, table.bearerKind, table.actionability, table.id)
...
export const responsibilityTemporalFacts = pgTable(
  ...
  {
    id: uuid('id').default(sql`pg_catalog.gen_random_uuid()`).primaryKey(),
    responsibilityId: uuid('responsibility_id').notNull().references(() => responsibilities.id, {onDelete: 'cascade'}),
    ...
    currentnessStatus: text('currentness_status').notNull().default('ACCEPTED_CURRENT'),
    authorityStatus: text('authority_status'),
  ...
  (table) => [
    foreignKey({
      ...
      foreignColumns: [responsibilityObligationLegs.id, responsibilityObligationLegs.responsibilityId]
    }).onDelete('no action'),
    foreignKey({
    ...
    uniqueIndex('responsibility_temporal_current_parent_uq')
      ...
      .where(sql`${table.currentnessStatus} = 'ACCEPTED_CURRENT' AND ${table.obligationLegId} IS NULL AND ${table.expectedEventId} IS NULL`),
    uniqueIndex('responsibility_temporal_current_leg_uq')
      .on(table.responsibilityId, table.temporalKind, table.obligationLegId)
      .where(sql`${table.currentnessStatus} = 'ACCEPTED_CURRENT' AND ${table.obligationLegId} IS NOT NULL AND ${table.expectedEventId} IS NULL`),
    uniqueIndex('responsibility_temporal_current_event_uq')
      .on(table.responsibilityId, table.temporalKind, table.expectedEventId)
      .where(sql`${table.currentnessStatus} = 'ACCEPTED_CURRENT' AND ${table.expectedEventId} IS NOT NULL AND ${table.obligationLegId} IS NULL`),
    index('responsibility_temporal_current_date_idx')
      .on(table.temporalKind, table.resolvedDate, table.responsibilityId)
      .where(sql`${table.currentnessStatus} = 'ACCEPTED_CURRENT' AND ${table.valueKind} = 'DATE'`),
    index('responsibility_temporal_current_instant_idx')
      .on(table.temporalKind, table.resolvedAt, table.responsibilityId)
      .where(sql`${table.currentnessStatus} = 'ACCEPTED_CURRENT' AND ${table.valueKind} = 'INSTANT'`),
...
export const responsibilityFieldDecisions = pgTable(
  ...
  (table) => [
    check('responsibility_field_decisions_field_key_nonempty', sql`char_length(btrim(${table.fieldKey})) BETWEEN 1 AND 128`),
    check('responsibility_field_decisions_value_version_check', sql`${table.valueSchemaVersion} >= 1`),
    check('responsibility_field_decisions_basis_revision_check', sql`${table.basisEvidenceRevision} >= 0`),
    check('responsibility_field_decisions_status_check', sql`${table.decisionStatus} IN ('ACTIVE', 'SUPERSEDED')`),
    check(
...
export const responsibilityAdmissionReviews = pgTable(
  'responsibility_admission_reviews',
  {
    ...
    candidateSchemaVersion: smallint('candidate_schema_version').notNull().default(1),
  ...
  (table) => [
    unique('responsibility_admission_reviews_id_account_uq').on(table.id, table.connectedAccountId),
    unique('responsibility_admission_reviews_id_user_uq').on(table.id, table.userId),
    ...
    // Drizzle 0.45.2 does not emit PostgreSQL column-list SET NULL. Keep the
    // tenant owner intact and use explicit retention cleanup for AI runs.
    foreignKey({
      name: 'responsibility_admission_reviews_interpretation_run_user_fk',
      columns: [table.interpretationRunId, table.userId],
      foreignColumns: [aiInterpretationRuns.id, aiInterpretationRuns.userId]
    }).onDelete('no action'),
    check('responsibility_admission_reviews_status_check', sql`${table.reviewStatus} IN ('OPEN', 'RESOLVED')`),
    check(
      'responsibility_admission_reviews_resolution_check',
      sql`${table.resolution} IS NULL OR ${table.resolution} IN ('TRACK', 'DO_NOT_TRACK')`
    ),
    check('responsibility_admission_reviews_reason_codes_check', sql`cardinality(${table.reasonCodes}) >= 1`),
    check(
    ...
    check('responsibility_admission_reviews_candidate_version_check', sql`${table.candidateSchemaVersion} >= 1`),
    check('responsibility_admission_reviews_basis_revision_check', sql`${table.basisEvidenceRevision} >= 0`),
    check('responsibility_admission_reviews_aggregate_version_check', sql`${table.aggregateVersion} >= 1`),
    check(
...
export const responsibilityDomainEvents = pgTable(
  ...
  (table) => [
    unique('responsibility_domain_events_id_parent_uq').on(table.id, table.responsibilityId),
    ...
    check('responsibility_domain_events_reason_codes_check', sql`cardinality(${table.reasonCodes}) >= 1`),
    check('responsibility_domain_events_revision_check', sql`${table.basisEvidenceRevision} >= 0`),
    check(
    ...
    index('responsibility_domain_events_correlation_idx').on(table.correlationId, table.id),
...
export const responsibilityProvenanceRefs = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      foreignColumns: [responsibilityAdmissionReviews.id, responsibilityAdmissionReviews.userId]
    }).onDelete('cascade'),
    foreignKey({
      name: 'responsibility_provenance_refs_review_account_fk',
      columns: [table.admissionReviewId, table.connectedAccountId],
      foreignColumns: [responsibilityAdmissionReviews.id, responsibilityAdmissionReviews.connectedAccountId]
    }).onDelete('cascade'),
    foreignKey({
    ...
    foreignKey({
      ...
      foreignColumns: [responsibilityDomainEvents.id, responsibilityDomainEvents.responsibilityId]
    }).onDelete('no action'),
    check(
