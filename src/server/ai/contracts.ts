...
export const AI_INTERPRETATION_SCHEMA_VERSION = 3 as const;
...
export type ModelCommunicatedClaim = ModelProvenanced<{
  id: string;
  kind: 'ATTACHMENT_DELIVERED' | 'COMPLETION' | 'DELIVERY' | 'ACKNOWLEDGEMENT' | 'OTHER';
  value: ModelJSONValue;
}>;
...
export type ModelSemanticUnit = {
  ...
  communicatedClaims: ModelCommunicatedClaim[];
...
}, ['messageId', 'participantId', 'zone', 'excerpt', 'start', 'end']);
...
const communicatedClaimSchema = sourceProvenanced({
  id: stringSchema(128), kind: enumSchema(['ATTACHMENT_DELIVERED', 'COMPLETION', 'DELIVERY', 'ACKNOWLEDGEMENT', 'OTHER']), value: jsonValueSchema
}, ['id', 'kind', 'value']);
...
const semanticUnitSchema = objectSchema({
  ...
  obligationLegs: arraySchema(obligationLegSchema), expectedEvents: arraySchema(expectedEventSchema), temporalFacts: arraySchema(temporalFactSchema), completionCriteria: arraySchema(completionSchema), constraints: arraySchema(constraintSchema), pendingProposals: arraySchema(proposalSchema), communicatedClaims: arraySchema(communicatedClaimSchema), agreedFacts: arraySchema(agreedFactSchema), uncertainties: arraySchema(uncertaintySchema), riskDetails: arraySchema(riskSchema), assignmentSemantics: assignmentSchema, corrections: arraySchema(correctionSchema), terminalSignal: terminalSchema, sourceRefs: arraySchema(sourceRefSchema)
}, ['candidateUnitKey', 'materiality', 'operationalOutcome', 'identityRelation', 'obligationLegs', 'expectedEvents', 'temporalFacts', 'completionCriteria', 'constraints', 'pendingProposals', 'communicatedClaims', 'agreedFacts', 'uncertainties', 'riskDetails', 'assignmentSemantics', 'corrections', 'terminalSignal', 'sourceRefs']);

export const INTERPRETATION_RESPONSE_FORMAT: StructuredResponseFormat = {
  type: 'json_schema', name: 'lunowa_responsibility_interpretation_v3', strict: true,
  schema: objectSchema({
...
function sourceRefs(
  ...
  return value.map((item, index) => {
    ...
    exact(source, ['messageId', 'participantId', 'zone', 'excerpt', 'start', 'end'], `${label}[${index}]`);
    ...
    const participantId = optionalString(source.participantId, `${label}[${index}].participantId`, 128);
    ...
    return {messageId, ...(participantId ? {participantId} : {}), zone, ...(excerpt ? {excerpt} : {}), ...(start !== undefined ? {start} : {}), ...(end !== undefined ? {end} : {})};
  });
}

function assertParticipantIsCited(
  participantId: string,
  ...
  const email = allowedParticipantEmails.get(participantId);
  if (!email) throw new AIContractError(`${label} names an unauthorized participant`);
  if (!refs.some((ref) => ref.participantId === participantId && messageParticipantEmails.get(ref.messageId)?.has(email))) {
    throw new AIContractError(`${label} participant identity is not associated with its cited message`);
  }
}
...
function validateParticipantRefs(
  ...
  for (const [index, ref] of refs.entries()) {
    ...
    const email = allowedParticipantEmails.get(ref.participantId);
    if (!email || !messageParticipantEmails.get(ref.messageId)?.has(email)) {
      throw new AIContractError(`${label}[${index}] participant is not associated with its message`);
    }
  }
  return [...refs];
}

function mapSourceRefs(refs: readonly ModelSourceRef[], evidenceKind: ProvenanceInput['evidenceKind'] = 'COMMUNICATED_CLAIM'): ProvenanceInput[] {
  return refs.map((ref) => ({
    evidenceKind, messageId: ref.messageId,
    ...(ref.excerpt ? {sourceExcerptShort: ref.excerpt} : {}),
    sourceLocator: {messageId: ref.messageId, zone: ref.zone, ...(ref.participantId ? {participantId: ref.participantId} : {}), ...(ref.start !== undefined ? {start: ref.start} : {}), ...(ref.end !== undefined ? {end: ref.end} : {})}
  }));
}
...
function hasCurrentTurnSource(refs: readonly ModelSourceRef[]): boolean {
  return refs.some((ref) => ref.zone === 'AUTHORED_CURRENT');
}
...
function validateSemanticUnit(
  ...
  const communicatedClaims = list('communicatedClaims', (value, nestedLabel) => { const item = record(value, nestedLabel); exact(item, ['id', 'kind', 'value', 'sourceRefs'], nestedLabel); const refs = refsFor(item, nestedLabel); const encoded = encodedJsonValue(item.value, `${nestedLabel}.value`); assertEncodedValueGrounded(encoded, refs, `${nestedLabel}.value`, authorizedMessageBodies); return {id: stringValue(item.id, `${nestedLabel}.id`, 128), kind: enumValue(item.kind, ['ATTACHMENT_DELIVERED', 'COMPLETION', 'DELIVERY', 'ACKNOWLEDGEMENT', 'OTHER'] as const, `${nestedLabel}.kind`), value: encoded, sourceRefs: refs}; });
  ...
  const uncertainties = list('uncertainties', (value, nestedLabel) => { const item = record(value, nestedLabel); exact(item, ['id', 'fieldKey', 'reasonCode', 'material', 'reviewRequired', 'candidateRefs', 'sourceRefs'], nestedLabel); const refs = item.candidateRefs === undefined || item.candidateRefs === null ? undefined : (() => { if (!Array.isArray(item.candidateRefs)) throw new AIContractError(`${nestedLabel}.candidateRefs must be an array`); return item.candidateRefs.map((ref, index) => stringValue(ref, `${nestedLabel}.candidateRefs[${index}]`, 128)); })(); const reasonCode = stringValue(item.reasonCode, `${nestedLabel}.reasonCode`, 128); if (reasonCode === 'PROVIDER_CONTRADICTION') throw new AIContractError(`${nestedLabel}.PROVIDER_CONTRADICTION is trusted application evidence, not a model claim`); return {id: stringValue(item.id, `${nestedLabel}.id`, 128), fieldKey: stringValue(item.fieldKey, `${nestedLabel}.fieldKey`, 128), reasonCode, material: booleanValue(item.material, `${nestedLabel}.material`), reviewRequired: booleanValue(item.reviewRequired, `${nestedLabel}.reviewRequired`), ...(refs ? {candidateRefs: refs} : {}), sourceRefs: refsFor(item, nestedLabel)}; });
  ...
  const assignment = item.assignmentSemantics === undefined || item.assignmentSemantics === null ? undefined : (() => {
    const value = record(item.assignmentSemantics, `${label}.assignmentSemantics`);
    exact(value, ['id', 'shape', 'candidateParticipantIds', 'selectedParticipantId'], `${label}.assignmentSemantics`);
    const shape = enumValue(value.shape, ['ANY_OF', 'ALL_OF', 'UNSPECIFIED_GROUP'] as const, `${label}.assignmentSemantics.shape`);
    if (!Array.isArray(value.candidateParticipantIds)) throw new AIContractError(`${label}.assignmentSemantics.candidateParticipantIds must be an array`);
    const ids = value.candidateParticipantIds.map((id, index) => stringValue(id, `${label}.assignmentSemantics.candidateParticipantIds[${index}]`, 128));
    if (ids.some((id) => !allowedParticipantIds.has(id))) throw new AIContractError(`${label}.assignmentSemantics contains an unauthorized participant`);
    ids.forEach((id) => {
      // A connected user may be one of several unresolved ANY_OF candidates
      // (for example USER=Sato), but the model may not select that user as the
      // unique bearer or turn the group into an ordinary counterparty leg.
      if (allowedParticipantRoles.get(id) === 'CONNECTED_USER' && shape !== 'ANY_OF') {
        throw new AIContractError(`${label}.assignmentSemantics cannot place the connected user in a unique/group assignment`);
      }
      if (allowedParticipantRoles.get(id) !== 'CONNECTED_USER') assertCounterpartyParticipant(id, `${label}.assignmentSemantics`, allowedParticipantRoles);
      assertParticipantIsCited(id, refs, `${label}.assignmentSemantics`, allowedParticipantEmails, messageParticipantEmails);
    });
    const selected = optionalString(value.selectedParticipantId, `${label}.assignmentSemantics.selectedParticipantId`, 128);
    if (selected && !allowedParticipantIds.has(selected)) throw new AIContractError(`${label}.assignmentSemantics.selectedParticipantId is unauthorized`);
    if (selected) {
      assertCounterpartyParticipant(selected, `${label}.assignmentSemantics.selectedParticipantId`, allowedParticipantRoles);
    ...
    return {id: stringValue(value.id, `${label}.assignmentSemantics.id`, 128), shape, candidateParticipantIds: ids, ...(selected ? {selectedParticipantId: selected} : {})};
  })();
  ...
  const decisionBearingRefs = [
    ...obligationLegs.flatMap((entry) => entry.sourceRefs), ...expectedEvents.flatMap((entry) => entry.sourceRefs),
    ...temporalFacts.flatMap((entry) => entry.sourceRefs), ...completionCriteria.flatMap((entry) => entry.sourceRefs),
    ...constraints.flatMap((entry) => entry.sourceRefs), ...proposals.flatMap((entry) => entry.sourceRefs),
    ...communicatedClaims.flatMap((entry) => entry.sourceRefs), ...agreedFacts.flatMap((entry) => entry.sourceRefs),
    ...corrections.flatMap((entry) => entry.sourceRefs), ...(terminal?.sourceRefs ?? [])
  ];
  if (materiality === 'MATERIAL' && decisionBearingRefs.length > 0 && !hasCurrentTurnSource(decisionBearingRefs)) {
    throw new AIContractError(`${label} material semantics must be grounded in current-turn communicative evidence`);
  }
...
function mapCandidateUnit(unit: ModelSemanticUnit): CandidateResponsibilitySemantics {
  ...
  const unitEvidenceKind: ProvenanceInput['evidenceKind'] = unit.identityRelation?.kind === 'SAME_UNSATISFIED_OUTCOME'
    ...
    : unit.identityRelation?.kind === 'REPLACES'
      ...
      : 'COMMUNICATED_CLAIM';
  const mapTerminal = (signal: ModelSemanticUnit['terminalSignal']): CandidateTerminalSignal | undefined => signal ? {kind: signal.kind, provenance: mapSourceRefs(signal.sourceRefs, signal.kind === 'COMPLETED' ? 'EXPLICIT_COMPLETION' : signal.kind === 'NONE' ? 'COMMUNICATED_CLAIM' : 'COUNTERPART_EXPLICIT_CLOSURE')} : undefined;
  ...
  return {
    ...
    provenance: mapSourceRefs(unit.sourceRefs, unitEvidenceKind)
  };
}
