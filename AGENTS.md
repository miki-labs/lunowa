...
- The isolated ACP worker has no general web/GitHub/provider/private-registry access. For this public pnpm repository its network exception is bounded to the public npm registry, and qualification requires that it cannot reach arbitrary controller/host-local or private-network destinations. `allow_local_binding = false` is **not** a blanket claim that the worker cannot create a local listener; the trust boundary is unauthorized-destination/credential/Git-metadata isolation.
